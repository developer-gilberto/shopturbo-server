...
export async function getAccessToken(req: ExtendedRequest, res: Response) {
    ...
    try {
        ...
        if (!storedToken.data) {
            ...
            // atualiza o accessToken para versao definitiva antes de salvar.
            // porque o primeiro accessToken eh temporario.
            const result = await shopeeApiAuth.requestNewShopeeApiAccessToken(
                Number(safeData.data.shop_id),
            ...
            if (result.error) {
                res.status(500).json({
                    error: true,
                    message:
                        "An error occurred while trying to update accessToken from the Shopee API. " +
                        result.data.message,
                });
                return;
            }
            ...
            const accessTokenData = await tokenRepo.save({
                ...
                expireIn: result.data.expire_in!,
            });
        ...
        if (expiredToken) {
            ...
            res.status(200).json({
                error: false,
                message:
                    "Access token updated. New access token successfully obtained.",
                data: {
